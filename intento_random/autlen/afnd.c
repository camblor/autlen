...
int AFNDTipoEstadoEn(AFND *p_afnd, int pos)
{
    return _afnd_ofus_obscure(p_afnd->_afnd_ofus_dog_obscure[pos]);
}
/*
Creacion de una estructura intermedia con los nuevos estados del AFD, y las transiciones.
Para cada nuevo estado se debe guardar los estados del AFND original de los que esta compuesto.
*/
AFND *AFNDTransforma(AFND *afnd)
{

    char **a;
    /* int longitud_lenguaje = afnd->num_simbolos; */
    /* int numero_estados = afnd->num_estados; */
    ...
    /* Estado inicial */
    int inicial = AFNDIndiceEstadoInicial(afnd);
    ...
    /* Encontrar estado inicial transformado (Posibilidad varios) */
    for(estado=0; estado<AFNDNumEstados(afnd);estado++ ){
        if(AFNDCierreLTransicionIJ(afnd, inicial, estado)){
    ...
    inicialDeterministaIndex = malloc(sizeof(int) * numIniciales);
    ...
    la unión de los estados del autómata original en los que el AFND podía estar a la vez.

    Ejemplo:
    Si el cierre transitivo del estado inicial es q0 y q1, el estado inicial del AFD será q0q1.
    En este caso el criterio para nombrar los nuevos estados ha sido concatenarlos en orden alfabético.
    Se puede usar otro criterio para los nuevos nombres, siempre y cuando no genere dos nombres distintos
